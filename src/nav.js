        ...
        <div onClick={()=> setopen(!open)} className='w-7 h-7 right-8 top-6 cursor-pointer md:hidden text-white absolute'>
          ...
          <ul className={`md:flex md:item-center ms-4 md:pb-0 absolute md:static md:z-auto z-10 left-0 w-full md:w-auto mt-4 md:mt-0 md:pl-9 bg-slate-400 sm:bg-slate-400  md:bg-transparent transition-all duration-500 ease-in ${open ? "top-12" : "top-[-490px]"}`}>
                  ...
                  <li  className="md:ml-8 md:my-0 my-7 font-semibold flex">
                    <button className="block w-full max-w-xs mx-auto bg-white  focus:bg-indigo-700 text-slate-900 rounded-lg  py-1 font-semibold me-3 px-3" onClick={signupHandler}>Signup</button>
                    <button className="block w-full max-w-xs mx-auto bg-white focus:bg-indigo-700 text-slate-900 rounded-lg  py-1 font-semibold ms-3 px-3" onClick={loginHandler}>login</button>
                  </li>
